...
public class Main {
    public static void main(String[] args) {
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            if (option == 1) {
                LocalDate date = InputValidator.getDate(scanner);
                ...
                double amount = InputValidator.getCustomAmount(scanner);
    ...
    private static String getUserInput(Scanner scanner, String message, String date, boolean isSourceCurrency) {
        ...
        } while ((!currencyCode.isEmpty() && !InputValidator.isCurrencyAvailable(currencyCode, date)) || (isSourceCurrency && currencyCode.isEmpty()));
        return currencyCode;
    }
    ...
    private static void displayResultsFromFile(String filePath, CurrencyExchangeCalculateService currencyExchangeCalculateService) {
        ...
        if(!fileHandler.readExchangeRates().isEmpty()) {
